...
"""
"""
scaler = StandardScaler().fit(my_data_train)
my_data_train = scaler.transform(my_data_train)
my_data_test = scaler.transform(my_data_test)

"""
parameters = param_grid = [{'C': [1,10,100,1000], 'gamma': [0.001, 0.0001,0.00001,'scale','auto'],'kernel':['rbf','sigmoid','poly']}]

clf =svm.SVC(class_weight='balanced',kernel='rbf',C=100,gamma=1e-05)
#search = GridSearchCV(clf,parameters,cv=3,scoring='accuracy',n_jobs=-1).fit(my_data_train, Label_train)
