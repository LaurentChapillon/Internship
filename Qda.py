# -*- coding: utf-8 -*-
"""
Created on Fri Apr 24 16:15:21 2020
...
for i in range(0,np.size(Label)):
    if  Label[i-Loop]==1:
        my_data = np.delete(my_data, (i-Loop), axis=0)
        Label = np.delete(Label, (i-Loop), axis=0)
        Loop=Loop+1
"""
Loop=0;
...
qda = QuadraticDiscriminantAnalysis(priors=None,reg_param=0.8)
X_lda = qda.fit(my_data_train, Label_train)
X_lda_test = qda.predict(my_data_test)
X_co=confusion_matrix(Label_test, X_lda_test)
error = np.mean( X_lda_test != Label_test )
